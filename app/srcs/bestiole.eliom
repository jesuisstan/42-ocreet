...
let make_bestiole_ill bestiole =
	...
	bestiole.got_infected_at <- Some (Utils.get_time ()) ;
	let img_src = ( match n with
		| 0 -> bestiole.state <- Beserk ; bestiole.full_size_at <- Some (Utils.get_time () +. 7.0) ; "./images/bestiole_beserk.png"
...
let update_speed bestiole =
	let multiplier = match bestiole.state with
		...
		| Naughty -> 1.15
	in
	let time_speedup = (Utils.get_time () -. bestiole.start_time) /. 120.0 in
...
let next_coords bestiole =
	...
	| Some updated_at -> (
		let nb_pixels_decal = bestiole.speed *. (Utils.get_time () -. updated_at) in
...
let change_rotation_if_ok bestiole =
	...
	| Naughty -> ()
	| _ -> (
		if Utils.get_time () >= bestiole.change_rotation_at then (
...
let rec bestiole_thread bestiole =
	...
	| _ -> (
		Js_of_ocaml_lwt.Lwt_js.sleep 0.01 >>= fun () ->
			...
			| true -> ( bestiole.updated_at <- Some (Utils.get_time ()) ;
